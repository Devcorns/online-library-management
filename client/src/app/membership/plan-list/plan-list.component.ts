    ...
    <div class="card stat-card mb-4">
      ...
      <div class="card-body">
        <form [formGroup]="form" (ngSubmit)="submit()" class="row g-2">
          <div class="col-md-3"><input class="form-control" formControlName="name" placeholder="Plan name"></div>
          <div class="col-md-2">
          ...
          <div class="col-md-2"><input type="number" class="form-control" formControlName="durationInDays" placeholder="Duration (days)"></div>
          <div class="col-md-1"><input type="number" class="form-control" formControlName="fee" placeholder="Fee ₹"></div>
          <div class="col-md-2">
            <input type="number" class="form-control" formControlName="gracePeriodDays" placeholder="Grace days">
          </div>
          <div class="col-md-1">
            <input type="number" class="form-control" formControlName="finePerDay" placeholder="Fine/day ₹">
          </div>
          <div class="col-md-1 d-flex gap-1">
            <button class="btn btn-primary flex-fill" type="submit">{{editing ? 'Update' : 'Add'}}</button>
            <button *ngIf="editing" class="btn btn-secondary flex-fill" type="button" (click)="cancelEdit()">✕</button>
          </div>
        </form>
      </div>
    </div>
...
export class PlanListComponent implements OnInit {
  ...
  edit(p: any) {
    this.editing = true; this.editId = p._id;
    this.form.patchValue(p);
  }
  cancelEdit() { this.editing = false; this.editId = ''; this.form.reset({ planType: 'monthly', durationInDays: 30, gracePeriodDays: 5, finePerDay: 10 }); }
  deletePlan(id: string) {
    this.svc.deletePlan(id).subscribe({ next: () => { this.toast.success('Plan removed.'); this.load(); }, error: (e: any) => this.toast.error(e.error?.message) });
  }
}
